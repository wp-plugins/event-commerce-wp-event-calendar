<?php
/*
Plugin Name: Wordpress Event Commerce
Plugin URI: http://www.eventmanagement.tourcircle.com
Description: Event Booking Management System
Version: 1.0
Author: SolverCircle
Author URI: http://www.solvercircle.com
Text Domain: evntgen-scbooking
Domain Path: /i18n/languages/
*/
define('GENSCEVENT_PLUGIN_URL', plugins_url('',__FILE__));
define("EVNTGEN_SC_BASE_URL", WP_PLUGIN_URL.'/'.plugin_basename(dirname(__FILE__)));
define('GENSCEVENT_DIR', plugin_dir_path(__FILE__) );
define('EVNTGEN_BOOKED_BG_COLOR','138219') ;

$scevent_calendar_page = get_page_by_title('SC Event Calendar');

$scevent_calendar_page_id= 0;
if(isset($scevent_calendar_page)){
  $scevent_calendar_page_id = $scevent_calendar_page->ID;
}
define('SCEVENTCALENDAR_PAGEID', $scevent_calendar_page_id);
...
include_once('operations/scevent_init.php');
//=================================================
...
function evntgen_create_custom_post_type() {
	register_post_type( 'evntgen_custom_event',
		array(
			'labels' => array(
				'name' => __('Events','evntgen-scbooking'),
				'singular_name' => __('Event','evntgen-scbooking'),
				'menu_name'=>__('Wordpress Event Commerce','evntgen-scbooking'),
				'all_items'=>__('Events','evntgen-scbooking'),
				'add_new_item'=>__('Add New Event','evntgen-scbooking'),
				'add_new'=> __('Add New Event','evntgen-scbooking'),
				'not_found'=>__('No events found.','evntgen-scbooking'),
				'search_items'=>__('Search Events','evntgen-scbooking'),
				'edit_item'=>__('Edit Event','evntgen-scbooking'),
				'view_item'=>__('View Event','evntgen-scbooking'),
				'not_found_in_trash'=>__('No Events found in trash','evntgen-scbooking')
			),
			'public' => true,
			'has_archive' => true,
...
function evntgen_shoppingcartjs(){
	//wp_enqueue_script( 'shoppingcartjs',GENSCEVENT_PLUGIN_URL.'/shoppingcart/js/shoppingcart.js',true);
...
function evntgen_get_events(){
  if(isset($_REQUEST)){
    ...
    $sql = "select * from ".$table_prefix."evntgen_scevents where event_id=".$event_id;
...
function evntgen_check_event_booking(){
  if(isset($_REQUEST)){
    ...
    if($hdneventbookingid != '' || $hdneventbookingid != NULL ){
      $sql = "select * from ".$table_prefix."evntgen_scevents where (".$event_cond.") and 
...
function evntgen_save_event_booking(){
  if ( count($_POST) > 0 ){ 
    ...
    if($hdneventid == "" || $hdneventid == NULL){
      $wpdb->insert($table_prefix.'evntgen_scevents',$values );
...
function evntgen_search_event(){
  ...
  $sql = "select * from ".$table_prefix."evntgen_scevents where email='".$search_text."' or phone='".$search_text."' or schedule='".$search_text."' or date='".$search_text."'";
  $result = $wpdb->get_results($sql);
  $msg = "<div id='content_top'></div>";
  if(count($result)){
        $msg .= '<table class="wp-list-table widefat fixed bookmarks" cellspacing="0">
...
function evntgen_delete_event(){
  if ( count($_POST) > 0 ){ 
    global $table_prefix,$wpdb;
    $eventid = $_REQUEST['event_id'];	
    $aff_rows = $wpdb->query("delete from ".$table_prefix."evntgen_scevents where event_id='".$eventid."'");
    echo $aff_rows;		 
  }  
  exit;
}
...
function evntgen_get_events_by_schedule(){
  ...
  $sql = "select * from ".$table_prefix."evntgen_scevents where schedule like '%".$schedule."%'";
  $result = $wpdb->get_results($sql);
  echo json_encode($result);
  exit;
}
